...
エージェントを介してバックエンドメンバーとしてジョイン  
（2022年6月よりバックエンドチームのリーダー兼エンジニア）

オンライン指導塾のポータルサイトの開発を行う。  
講師と生徒の紐づけ、授業進捗の管理、宿題の管理などの機能を実装。  
バックチームのリーダーとして、開発フローの見直しや、メンバーの能力向上のための勉強会開催なども行う  

主な業務内容

- DDD（ドメイン駆動設計）に沿ってドメインオーナーにヒアリングしながらモデリング、リポジトリパターンでの実装
- PHP 8.0 から 8.2 （Laravel 8.x から 10.x）へのアップグレード
- 社内勉強会の開催 (テーマ：オブジェクト指向、モデリング、テスト)

<BR>

チームメンバー：8名 （うちバックエンド4名）: 2023/11現在

【Skills】  
PHP8.0 -> 8.2 (Laravel 8.x -> 10.x) / Next.js / GitHub Actions / Docker / AWS
  """
